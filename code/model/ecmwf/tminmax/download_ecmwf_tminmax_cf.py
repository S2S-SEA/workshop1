...
call("rm -rf " + dest_dir + "*_" + vsuffx[vtype] + "_*_cf.nc", shell=True)
...
for i in range(0, len(init_date)):
    server.retrieve({
        ...
        "area": "30/80/-20/150", 
        "stream": "enfh",
        "target": dest_dir + "ECMWF_" + vsuffx[vtype] + "_2016-" + init_date[i] + "_cf.grib",
        "time": "00:00:00",
        "type": "cf",
    })
     
    # Convert from grib to netcdf using ECMWF Grib API
    call("grib_to_netcdf -M -I method,type,stream,refdate -T -o " + dest_dir + "ECMWF_" + vsuffx[vtype] + "_2016-" + init_date[i] + "_cf.nc " + dest_dir + "ECMWF_" + vsuffx[vtype] + "_2016-" + init_date[i] + "_cf.grib", shell=True)
    # Remove the grib file after download
    call("rm -rf " + dest_dir + "ECMWF_" + vsuffx[vtype] + "_2016-" + init_date[i] + "_cf.grib", shell=True)
