...
#Creating the interimWeekly file. format is [year, week (based on initdate file), lat, lon]
    ...
    for year in range(startYear, endYear+1):
        ...
        if year == startYear :
            interimWeekly = np.empty([len(initDate),endYear+1-startYear, len(lats), len(lons)])
            intDate = []
...
def processEcmwfTemp(ec_input, model_initial_date, model_step ,initMonth, initDate, startYear, endYear, type):
 #Input: 
