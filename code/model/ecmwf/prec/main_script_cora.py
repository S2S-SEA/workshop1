#This main script processes the downloaded ECMWF and TRMM dataset to generate the correlation of anomalies (CORA) for each individual model lead time. 
#Self-defined functions "read_ecmwf", "read_trmm" and "plot_figure_cora" are used to read ECMWF and TRMM dataset and for final visualization.
    ...
    for i_date in range(0,len(model_initial_date)):
        ...
        for i_year in range(start_year,end_year+1):
            ...
            if start_date.month == int(target_month) and end_date.month == int(target_month):
               ...
               sum_ec_trmm = sum_ec_trmm + (ec_week_year - ec_week_climatology)*(trmm_week_year - trmm_week_climatology);
               sum_ec = sum_ec + (ec_week_year - ec_week_climatology)**2;
               sum_trmm = sum_trmm + (trmm_week_year - trmm_week_climatology)**2;

    #Calculate CORA
    cora = sum_ec_trmm/(sum_ec**(1.0/2)*sum_trmm**(1.0/2));

    #Call function "plot_figure_cora" to plot CORA
    plot_figure_cora.plot_figure(cora,ec_lat,ec_lon,model_date[:2],str(i_step+1));
