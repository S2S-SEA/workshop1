...
if plot_figure == True:
   ...
   s2s_utility.plot_processing(trmm_climatology[target_week,:,:],trmm_lat,trmm_lon,lat_down,lat_up,lon_left,lon_right,grid_lat,grid_lon,data_range,title_str,name_str,'Climatology')
   ...
   s2s_utility.plot_processing(trmm_average[target_week,target_year-start_year,:,:],trmm_lat,trmm_lon,lat_down,lat_up,lon_left,lon_right,grid_lat,grid_lon,data_range,title_str,name_str,'Average')

   data_range = [-10,10]
   title_str = 'TRMM Rainfall Anomaly' + '\n' + str(target_year) + ' ' + start_date + '-' + end_date
   name_str = 'TRMM_' + str(target_year) + '_' + start_date + '-' + end_date + '_Anomaly.png'
   s2s_utility.plot_processing(trmm_anomaly[target_week,target_year-start_year,:,:],trmm_lat,trmm_lon,lat_down,lat_up,lon_left,lon_right,grid_lat,grid_lon,data_range,title_str,name_str,'Anomaly')
