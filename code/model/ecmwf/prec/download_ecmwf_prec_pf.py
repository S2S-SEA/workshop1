...
fall within Nov for perturbed forecast ('pf'). To change to other
...
for i in range(0, len(init_date)):
    server.retrieve({
        ...
        "area": "30/80/-20/150", 
        "stream": "enfh",
        "target": dest_dir + "ECMWF_prec_2016-" + init_date[i] + "_pf.grib",
        "time": "00:00:00",
        "type": "pf",
    })
    
    # Convert from grib to netcdf using ECMWF Grib API 
    call("grib_to_netcdf -M -I method,type,stream,refdate -T -o " + dest_dir + "ECMWF_prec_2016-" + init_date[i] + "_pf.nc " + dest_dir + "ECMWF_prec_2016-" + init_date[i] + "_pf.grib", shell=True)
    # Remove the grib file after download
    call("rm -rf " + dest_dir + "ECMWF_prec_2016-" + init_date[i] + "_pf.grib", shell=True)
