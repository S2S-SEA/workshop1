...
#-------------------------------------------------------------------
#This part is to calculate ECMWF rainfall climatlogy/average/anomaly
#-------------------------------------------------------------------
...
for i_date in range(0,len(model_initial_date)):
    ...
    #Read ECMWF dataset
    ec_time,ec_step,ec_data = s2s_utility.read_ec(cur_ec_path)
...
if data_output == True:
   ...
   s2s_utility.write_ec(ec_output,ec_filename,ec_climatology,ec_step,ec_week,ec_year,ec_lat,ec_lon,'Climatology')

   ec_filename = 'ECMWF_' + calendar.month_abbr[target_month] + '_Average_Weekly.nc'
   s2s_utility.write_ec(ec_output,ec_filename,ec_average,ec_step,ec_week,ec_year,ec_lat,ec_lon,'Average')

   ec_filename = 'ECMWF_' + calendar.month_abbr[target_month] + '_Anomaly_Weekly.nc'
   s2s_utility.write_ec(ec_output,ec_filename,ec_anomaly,ec_step,ec_week,ec_year,ec_lat,ec_lon,'Anomaly')
...
if plot_figure == True:
   ...
   for i_step in range(0,model_step):
       ...
       s2s_utility.plot_fig(ec_climatology[i_step,target_week,:,:],ec_lat,ec_lon,lat_down,lat_up,lon_left,lon_right,grid_lat,grid_lon,data_range,title_str,name_str,'Climatology')
       ...
       s2s_utility.plot_fig(ec_average[i_step,target_week,target_year-start_year,:,:],ec_lat,ec_lon,lat_down,lat_up,lon_left,lon_right,grid_lat,grid_lon,data_range,title_str,name_str,'Average')

       data_range = [-10,10]
       title_str = 'ECMWF Rainfall Anomaly' + '\n' + str(target_year) + ' ' + start_date + '-' + end_date + ' (LT' + str(i_step+1) + ')'
       name_str = 'ECMWF_' + str(target_year) + '_' + start_date + '-' + end_date + '_' + 'LT' + str(i_step+1) + '_Anomaly.png'
       s2s_utility.plot_fig(ec_anomaly[i_step,target_week,target_year-start_year,:,:],ec_lat,ec_lon,lat_down,lat_up,lon_left,lon_right,grid_lat,grid_lon,data_range,title_str,name_str,'Anomaly')
