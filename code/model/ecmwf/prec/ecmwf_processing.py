...
if plot_figure == True:
   ...
   for i_step in range(0,model_step):
       ...
       s2s_utility.plot_processing(ec_climatology[i_step,target_week,:,:],ec_lat,ec_lon,lat_down,lat_up,lon_left,lon_right,grid_lat,grid_lon,data_range,title_str,name_str,'Climatology')
       ...
       s2s_utility.plot_processing(ec_average[i_step,target_week,target_year-start_year,:,:],ec_lat,ec_lon,lat_down,lat_up,lon_left,lon_right,grid_lat,grid_lon,data_range,title_str,name_str,'Average')

       data_range = [-10,10]
       title_str = 'ECMWF Rainfall Anomaly' + '\n' + str(target_year) + ' ' + start_date + '-' + end_date + ' (LT' + str(i_step+1) + ')'
       name_str = 'ECMWF_' + str(target_year) + '_' + start_date + '-' + end_date + '_' + 'LT' + str(i_step+1) + '_Anomaly.png'
       s2s_utility.plot_processing(ec_anomaly[i_step,target_week,target_year-start_year,:,:],ec_lat,ec_lon,lat_down,lat_up,lon_left,lon_right,grid_lat,grid_lon,data_range,title_str,name_str,'Anomaly')
