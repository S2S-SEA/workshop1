    ...
    while plotting:
        ...
        for i in range(lead_times):
         ...
         #Define title and name convention
            title_str = 'Average Temperature LT'+str(i)+' '+str(weeks[j])+'/11/'+str(plot_year)+',  ECMWF HindCast'
            name_str = 'ecmwf_' +"LT"+str(i)+'for date ' +str(plot_year)+'-'+str(month) +str(weeks[j])+'_' + 'Average'+'.png'
            s2s.plot_figure(ec_weekly[i,j,k, :, :],ec_lat,ec_lon,[240,300],title_str,name_str,'Average')
            title_str = 'Temperature Climatology LT'+str(i)+' '+str(weeks[j])+'/11/'+str(plot_year)+',  ECMWF HindCast'
            name_str = 'ecmwf_' +"LT"+str(i)+'for date ' +str(plot_year)+'-'+str(month) +str(weeks[j])+'_' + 'Climatology'+'.png'
            s2s.plot_figure(ec_clim[i,j, :, :],ec_lat,ec_lon,[240,300],title_str,name_str,'Climatology')
            title_str = 'Temperature Anomaly LT'+str(i)+' '+str(weeks[j])+'/11/'+str(plot_year)+',  ECMWF HindCast'
            name_str = 'ecmwf_' +"LT"+str(i)+'for date ' +str(plot_year)+'-'+str(month) +str(weeks[j])+'_' + 'Anomaly'+'.png'
