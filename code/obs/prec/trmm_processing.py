...
#------------------------------------------------------------------
#This part is to calculate TRMM rainfall climatlogy/average/anomaly
#------------------------------------------------------------------
...
#Read TRMM dataset
trmm_time,trmm_lat,trmm_lon,trmm_data = s2s_utility.read_trmm(cur_trmm_path)
...
if data_output == True:
   ...
   s2s_utility.write_trmm(trmm_output,trmm_filename,trmm_climatology,trmm_week,trmm_year,trmm_lat,trmm_lon,'Climatology')

   trmm_filename = 'TRMM_' + calendar.month_abbr[int(week_date[:2])] + '_Average_Weekly.nc'
   s2s_utility.write_trmm(trmm_output,trmm_filename,trmm_average,trmm_week,trmm_year,trmm_lat,trmm_lon,'Average')

   trmm_filename = 'TRMM_' + calendar.month_abbr[int(week_date[:2])] + '_Anomaly_Weekly.nc'
   s2s_utility.write_trmm(trmm_output,trmm_filename,trmm_anomaly,trmm_week,trmm_year,trmm_lat,trmm_lon,'Anomaly')
...
if plot_figure == True:
   ...
   s2s_utility.plot_fig(trmm_climatology[target_week,:,:],trmm_lat,trmm_lon,lat_down,lat_up,lon_left,lon_right,grid_lat,grid_lon,data_range,title_str,name_str,'Climatology')
   ...
   s2s_utility.plot_fig(trmm_average[target_week,target_year-start_year,:,:],trmm_lat,trmm_lon,lat_down,lat_up,lon_left,lon_right,grid_lat,grid_lon,data_range,title_str,name_str,'Average')

   data_range = [-10,10]
   title_str = 'TRMM Rainfall Anomaly' + '\n' + str(target_year) + ' ' + start_date + '-' + end_date
   name_str = 'TRMM_' + str(target_year) + '_' + start_date + '-' + end_date + '_Anomaly.png'
   s2s_utility.plot_fig(trmm_anomaly[target_week,target_year-start_year,:,:],trmm_lat,trmm_lon,lat_down,lat_up,lon_left,lon_right,grid_lat,grid_lon,data_range,title_str,name_str,'Anomaly')
